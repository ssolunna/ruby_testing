...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      ...
      it 'returns 7' do
        expect(game_seven.make_guess).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:game_nine) { described_class.new(8, 9, number_guessing) }

      it 'returns 8' do
        expect(game_nine.make_guess).to eq(8)
      end
    end
    ...
    context 'when min is 0 and max is 3' do
      ...
      it 'returns 1' do
        expect(game_one.make_guess).to eq(1)
      end
    end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:number_four) { double('random_number', value: 4) }
      subject(:game_over) { described_class.new(0, 9, number_four, 4) }
    ...
    context 'when guess and random_number are not equal' do
      let(:number_four) { double('random_number', value: 4) }
      subject(:game_not_over) { described_class.new(0, 9, number_four, '7') }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:same_min_max_game) { described_class.new(5, 8, number_range, 7) }

      it 'updates min to the same value as max' do
        expect { same_min_max_game.update_range }.to change { same_min_max_game.min }.to(8)
      end

      it 'does not update max' do
        expect { same_min_max_game.update_range }.not_to change { same_min_max_game.max }
      end
    end
  end
end
